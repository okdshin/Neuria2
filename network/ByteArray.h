...
auto CreateByteArrayFromString(const std::string& str) -> ByteArray {
	return ByteArray(str.begin(), str.end());
}
auto CreateStringFromByteArray(const ByteArray& byte_array) -> std::string {
	return std::string(byte_array.begin(), byte_array.end());
}
auto OutputByteArray(std::ostream& os, const ByteArray& byte_array) -> std::ostream& {
