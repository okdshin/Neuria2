...
public:
    MessageHeader(){}
    explicit MessageHeader(ByteArraySize body_size) : body_size(body_size) {
		if(body_size < 253){
			header_size = 1;
		}
	...
	static auto IsEnableParse(const ByteArray& byte_array) -> bool {
		...
		if(byte_array.empty()){return false;}
...
inline auto operator<<(std::ostream& os, const MessageHeader& header) -> std::ostream& {
